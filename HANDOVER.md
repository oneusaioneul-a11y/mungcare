...
※ 2026-08-29 커밋 2건은 **로컬에만 있고 원격(origin) 미푸시** 상태일 수 있으니 `git push` 여부 확인.
...
4. **처리방침 개정 규칙**: 문구 변경 시 `store.js PRIVACY_VERSION` 상향 → 신규 가입자는 새 판으로
   동의 기록됨. 기존 회원 재동의 UI는 미구현(백로그)
5. **파트너 확인 운영**: 가입 업체의 사업자등록번호를 확인하고 `partners.partners.verified` 를
   수동으로 true 처리 (관리자 화면 없음 — Supabase Table Editor 사용)
...
- **관리자 도구 없음**: 신고(`community.reports`) 처리, 파트너 verified, 회원 관리 전부 Supabase 대시보드 수동
