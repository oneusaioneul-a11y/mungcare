...
- **완료 — 로컬(기기 저장) 기능 전부 이식됨**: 프로젝트 스캐폴드(iOS·Android), 두들 테마,
  **한국 표준 회원가입**(전체 동의 + 필수 3종[만14세·이용약관·개인정보] + 선택[마케팅] +
  전문 보기 시트, 동의 이력 판·일시·거부까지 보관)·로그인, 홈,
  **아이 프로필·체중**, **밥 기록**(MER·급여량 계산), **산책 기록**(크기·나이별 목표),
  **접종·구충**(스케줄 엔진), **약 챙기기**(재고 경고·투약 체크), **진료 기록**(누적 진료비),
  **알러지**(밥 기록 경고 연동), **화식 레시피**(위험 재료 10종 검사).
  계산 엔진(`health.dart`·`vaccine.dart`·`toxic.dart`)은 웹과 **같은 기대값으로 테스트되어 동치 보장**.
  단위 38건 + 시뮬레이터 E2E(가입→전 화면 전 구간) 통과.
...
- **남은 이식** (전부 서버 데이터라 **Supabase 셋업이 선행 조건**): 수다방(글·댓글·추천·별점·대화창),
  용품 리뷰, 파트너 디렉터리, 유기견 입양(공공데이터 프록시). 셋업 후 `CloudAuthService`·
  클라우드 저장소를 같은 인터페이스로 추가하면 기존 화면은 그대로 동작합니다.
