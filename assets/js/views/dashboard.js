...
export default {
  head: ctx => ({
    title: ctx.dog ? `${ctx.dog.name}의 오늘` : '대시보드',
    sub: ctx.dog ? `${ctx.dog.breed || '견종 미등록'} · ${H.ageLabel(ctx.dog.birth)}${ctx.dog.weight ? ` · ${ctx.dog.weight}kg` : ''}` : '반려견을 먼저 등록해 주세요'
  }),

  mount(root, ctx) {
    if (!ctx.dog) {
      root.innerHTML = `<div class="card">${empty('🐾', '아직 등록된 반려견이 없습니다.',
        '<a href="#/profile" class="btn btn-primary">반려견 등록하기</a>')}</div>`;
      return;
    }
    ...
    <div class="stack">
      ${ctx.alerts.length ? `
      <div class="card">
        <div class="card-head"><h2>🔔 확인이 필요한 항목</h2>
      ...
      </div>` : `<div class="alert ok"><span class="ai">✅</span><span><b>지금 급한 항목은 없습니다.</b><br>
        <span style="opacity:.85">오늘의 식사와 산책을 기록해 두면 추이를 볼 수 있어요.</span></span></div>`}

      <div class="grid g4">
        <div class="stat">
          <div class="k">오늘 섭취</div>
          ...
          <div class="d">권장 ${target ? num(target) + ' kcal' : '체중 등록 필요'}</div>
        </div>
        <div class="stat">
        ...
        <div class="stat">
          <div class="k">복용 중인 약</div>
          <div class="v">${activeMeds.length}<span class="u">종</span></div>
          <div class="d">${activeMeds.length ? esc(activeMeds.map(m => m.name).slice(0, 2).join(', ')) : '등록된 약 없음'}</div>
        </div>
        <div class="stat">
          ...
          <div class="d">${nextVax ? esc(nextVax.name) : '접종 일정 없음'}</div>
        </div>
      </div>

      <div class="grid g2">
        <div class="card">
          <div class="card-head"><h2>🐾 주간 산책</h2><div class="spacer"></div>
            <span class="hint">하루 목표 ${goal}분</span></div>
          <div class="weekchart">
        ...
        <div class="card">
          <div class="card-head"><h2>🍚 오늘의 식사</h2><div class="spacer"></div>
          ...
          ${mealsToday.length ? `<div>${mealsToday.map(m => `
            ...
            : empty('🥣', '오늘 기록된 식사가 없습니다.')}
        </div>
      </div>

      ${ctx.risk?.now?.length ? `
      <div class="card">
        <div class="card-head"><h2>🩺 지금 나이에 특히 살펴볼 점</h2><div class="spacer"></div>
      ...
      <p class="disclaimer">이 서비스가 제공하는 정보는 일반적인 참고 자료입니다. 이상 증상이 보이면 반드시 수의사의 진료를 받으세요.</p>
    </div>`;
    ...
    root.querySelector('[data-quick-walk]')?.addEventListener('click', () => {
      modal({
        ...
        body: field('날짜', inputEl('date', { type: 'date', value: t, required: true }))
            + `<div class="inline">${field('시간(분)', inputEl('minutes', { type: 'number', min: 0, value: 30, required: true }))}
               ${field('거리(km)', inputEl('km', { type: 'number', step: '0.1', min: 0 }))}</div>`
            + field('메모', inputEl('note', { placeholder: '컨디션, 배변 상태 등' })),
    ...
    root.querySelector('[data-quick-meal]')?.addEventListener('click', () => {
      modal({
        ...
        body: `<div class="inline">${field('날짜', inputEl('date', { type: 'date', value: t, required: true }))}
            ...
            + field('메뉴', inputEl('name', { required: true, placeholder: '예: 연어 사료, 닭가슴살 화식' }))
            + `<div class="inline3">
        ...
        onSubmit: f => { ctx.meals.add({ ...f, grams: f.grams ? +f.grams : null, kcal: f.kcal ? +f.kcal : 0 }); toast('식사를 기록했습니다.'); }
      });
    });
  }
};
...
function recentTimeline(ctx) {
  ...
  push(ctx.meals.raw().slice(-12), '🍚', r => ({ t: r.name, b: `${r.type || '사료'} ${r.kcal ? '· ' + r.kcal + 'kcal' : ''}` }));
  push(ctx.walks.raw().slice(-12), '🐾', r => ({ t: `산책 ${r.minutes}분`, b: r.km ? `${r.km}km ${r.note || ''}` : (r.note || '') }));
  push(ctx.medical.raw().slice(-12), '🏥', r => ({ t: r.title || '진료', b: `${r.hospital || ''} ${r.diagnosis || ''}`.trim() }));
  push(ctx.vaccines.raw().slice(-12), '💉', r => ({ t: r.label || r.code, b: r.hospital || '' }));
  const sorted = items.filter(x => x.date).sort((a, b) => b.date.localeCompare(a.date)).slice(0, 8);
  if (!sorted.length) return empty('📭', '아직 기록이 없습니다. 위의 빠른 기록 버튼으로 시작해 보세요.');
  return `<div class="tl">${sorted.map(x => `
    <div class="tl-item"><div class="dt">${fmtDate(x.date)}</div>
      <div class="tt">${x.icon} ${esc(x.t)}</div>
